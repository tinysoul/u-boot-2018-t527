...
int fb_save_para(unsigned int fb_id)
{
	...
	static unsigned int iova_array[FRAMEBUFFER_NUM * 2] = {0};
	...
	iova_array[fb_id * 2 + 0] = (uintptr_t)fb->cv->base;
	iova_array[fb_id * 2 + 1] = fb->cv->stride * fb->cv->height;
	hal_save_iova_to_kernel("sunxi-iova-reserve", iova_array, FRAMEBUFFER_NUM * 2);
