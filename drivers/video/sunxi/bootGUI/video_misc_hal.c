	...
	unsigned int *value, unsigned int defval)
{
	if (fdt_getprop_u32(working_fdt, node, name, value) < 0) {
		pr_error("get disp.%s fail. using defval=%u\n", name, defval);
		*value = defval;
	}
}

int hal_save_iova_to_kernel(char *name, unsigned int *array, unsigned int length)
{
	int ret = -1;
	u32 *d = NULL;
	int len = 0;
	...
	if (length % 2) {
		printf("%s length must be even, %d\n",
				__func__, length);
		return ret;
	}

	d = (u32 *)calloc(length * 2, sizeof(u32));
	if (!d) {
		printf("%s calloc error!\n", __func__);
		return ret;
	}

	for (i = 0; i < length / 2; i++, len += 16) {
		if (!array[2 * i + 1])
			break;

		d[4 * i + 0] = 0;
		d[4 * i + 1] = cpu_to_fdt32(array[2 * i + 0]);
		d[4 * i + 2] = 0;
		d[4 * i + 3] = cpu_to_fdt32(array[2 * i + 1]);
	...
	ret = fdt_setprop(working_fdt, node, name, d, len);
#else
	/* TODO */
	printf("%s TODO: support multicore boot\n", __func__);
#endif

	free(d);

	/* pr_msg("%s %s.%s code:%s\n", */
	/*         __func__, DISP_FDT_NODE, name, fdt_strerror(ret)); */
	return ret;
}
