...
static int twi_start(int bus_num)
{
	...
	if (i2c->status != I2C_START_TRANSMIT) {
		I2C_ERR("Status not is I2C_START_TRANSMIT! status = 0x%x\n", i2c->status);
		return SUNXI_I2C_FAIL;
	}
...
static int twi_restart(int bus_num)
{
	...
	if (i2c->status != I2C_RESTART_TRANSMIT) {
		I2C_ERR("Status not is I2C_RESTART_TRANSMIT! status = 0x%x\n", i2c->status);
		return SUNXI_I2C_FAIL;
	}
...
static int twi_send_slave_addr(int bus_num, u32 saddr,  u32 rw)
{
	...
	if (twi_wait_irq_flag(bus_num, time)) {
		I2C_DBG("twi wait clear irq flag timeout\n");
		return SUNXI_I2C_TOUT;
	}

	if ((rw == I2C_WRITE) && (i2c->status != I2C_ADDRWRITE_ACK)) {
		I2C_ERR("Status not is I2C_ADDRWRITE_ACK! status = 0x%x\n", i2c->status);
		return SUNXI_I2C_FAIL;
	} else if ((rw == I2C_READ) && (i2c->status != I2C_ADDRREAD_ACK)) {
		I2C_ERR("Status not is I2C_ADDRREAD_ACK! status = 0x%x\n", i2c->status);
		return SUNXI_I2C_FAIL;
	}
...
static int  twi_send_byte_addr(int bus_num, u32 byteaddr)
{
	...
	if (i2c->status != I2C_DATAWRITE_ACK) {
		I2C_ERR("Status not is I2C_DATAWRITE_ACK! status = 0x%x\n", i2c->status);
		return SUNXI_I2C_FAIL;
	}
	...
	if (data_count == 1) {
		...
		*data_addr = i2c->data;
		...
		if (i2c->status != I2C_DATAREAD_NACK) {
			I2C_ERR("Status not is I2C_DATAREAD_NACK! Status = 0x%x\n", i2c->status);
...
static int twi_stop(int bus_num)
{
	...
	while ((i2c->status != TWI_STAT_IDLE) && (--time))
		;
